    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* 2. 底部滑出面板 (Slide Up/Down) */}
          <motion.div 
            initial={{ y: "100%" }} // 初始位置：螢幕下方
            animate={{ y: 0 }}      // 進場位置：正常顯示
            exit={{ y: "100%" }}    // 離場位置：滑回下方
            transition={{ type: "spring", damping: 25, stiffness: 300 }} // 彈性動畫
            
            // ★ 手勢支援：向下滑動關閉
            drag="y"
            dragConstraints={{ top: 0 }} // 不能往上拉
            dragElastic={0.2}
            onDragEnd={(_, info) => {
              // 如果向下滑動超過 100px 或速度很快，就關閉
          ...
          >
            ...
            <div className="mt-8 text-center text-gray-400 text-sm pb-safe">
