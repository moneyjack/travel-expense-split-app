...
export const TripDashboard: React.FC<TripDashboardProps> = ({ trip, onViewExpense, onNavigateTripList }) => {
  ...
  const { t, i18n } = useTranslation(); // ★ 初始化翻譯
  ...
  const getDateLabel = (dateKey: string) => {
    ...
    if (date.toDateString() === today.toDateString()) return t('dashboard.date_labels.today');
    if (date.toDateString() === yesterday.toDateString()) return t('dashboard.date_labels.yesterday');
    
    // 根據當前語言切換日期格式
    return date.toLocaleDateString(i18n.language === 'zh' ? 'zh-TW' : 'en-US', { 
      weekday: 'short', 
      month: 'short', 
      day: 'numeric' 
    });
    ...
    <div className="space-y-6 pb-24 animate-in fade-in">
      ...
      <div className="flex justify-between items-center mb-2 px-1">
           <button onClick={onNavigateTripList} className="flex items-center text-gray-400 font-bold text-sm gap-1 hover:text-gray-600 transition-colors">
               <Icons.Back /> {t('dashboard.back_to_trips')}
           </button>

           <div className="flex gap-2">
               <button onClick={() => setShowShare(true)} className="p-2 bg-white rounded-full shadow-sm text-gray-500 hover:text-primary transition-colors active:scale-95">
                   <Icons.Share />
               </button>
               <button onClick={() => setShowSettings(true)} className="p-2 bg-white rounded-full shadow-sm text-gray-500 hover:text-primary transition-colors active:scale-95">
                   <Icons.Settings />
               </button>
           </div>
      </div>
      ...
      <div className="bg-primary rounded-3xl p-6 text-white shadow-lg shadow-indigo-500/30 relative overflow-hidden">
        ...
        <p className="text-indigo-100 text-sm font-medium mb-1 tracking-wide uppercase">{t('dashboard.total_expenses')}</p>
        ...
        <div className="mt-6 flex items-center justify-between">
             ...
             {displayMembers.slice(0, 4).map((m: any) => {
               ...
               return (
                 <div key={m.id} className={`relative flex flex-col items-center ${isMe ? 'z-10' : ''}`}>
                   {isHostMember && (
                     <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-20"><Icons.Crown /></div>
                   )}
          ...
          <div className="bg-white/20 backdrop-blur-md px-3 py-1 rounded-lg text-xs font-medium">
             {t('dashboard.receipts_count', { count: trip.expenses.length })}
          </div>
        </div>
      </div>
      ...
      {/* 交易列表 */}
      <div>
        <h3 className="text-lg font-bold text-gray-800 mb-4 px-2">{t('dashboard.recent_transactions')}</h3>
        {trip.expenses.length === 0 ? (
           <div className="flex flex-col items-center justify-center py-12 text-gray-400 bg-white rounded-3xl border border-dashed border-gray-200">
              <Icons.Receipt />
              <p className="mt-2 text-sm">{t('dashboard.no_receipts')}</p>
           </div>
        ) : (
               ...
               <div key={dateKey}>
                 ...
                 <div className="space-y-3">
                   {expenses.map((exp: Expense) => {
                     const payer = trip.members.find((m: any) => m.id === exp.payerId);
                     return (
                       <div 
                        key={exp.id} 
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                              ...
                              <p className="text-xs text-gray-400 font-medium mt-0.5">
                                {exp.is_settled 
                                  ? t('dashboard.expense.settled') 
                                  : t('dashboard.expense.paid_by', { name: payer?.name || t('dashboard.expense.unknown_payer') })}
                              </p>
                          </div>
                          </div>
